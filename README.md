# Maths and Trigonometry Assignment
...
Open `HelloWorld.vcxproj` in Visual Studio, build the project, and press **F5**. The project requires `Play.h` and its associated data files.

## Approach and game development examples

I broke each mathematical expression into smaller steps, then translated those steps into C++ using variables, loops, and functions. I tested the calculations with simple values, such as `x = 3` and `y = 4`, where the length √(x² + y²) should be `5`.

These concepts are useful in game development:

- **Distance and length:** Calculate how far a player is from an enemy or item.
